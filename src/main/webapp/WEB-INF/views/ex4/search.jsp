...
<head>
    ...
    <style>
        ...
        .badge-free { padding: 4px 8px; border-radius: 12px; background: #e8f7e8; font-weight: bold; }
        .sold-out { color: #c62828; font-weight: bold; }
        .low-stock { color: #ef6c00; font-weight: bold; }
        .available { color: #2e7d32; font-weight: bold; }
        .btn-book { padding: 6px 10px; text-decoration: none; border: 1px solid #1976d2; border-radius: 4px; }
        .btn-disabled {
            padding: 6px 10px;
        ...
        .footer-box { margin-top: 22px; padding-top: 12px; border-top: 1px solid #ddd; }
    </style>
</head>
<body>
...
<p>Tìm thấy ${empty events ? 0 : fn:length(events)} sự kiện</p>
...
<div class="footer-box">
    <c:if test="${not empty events}">
        <div>Tên sự kiện đầu tiên: ${fn:toUpperCase(events[0].name)}</div>
    </c:if>
    <div>Số ký tự của từ khóa tìm kiếm: ${fn:length(keyword)} ký tự</div>
</div>
